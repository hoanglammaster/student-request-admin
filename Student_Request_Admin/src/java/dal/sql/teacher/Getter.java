 ...
 *
 */
class Getter extends AbstractGetter<Teacher> implements TeacherGettable {

    public Getter(Connections connectioner, Convertible converter) {
        super(connectioner, converter);
    }

    @Override
    public Teacher get(int id) {
        String query = "SELECT TeacherId,CONCAT(FirstName,' ',MidleName,' ',LastName) AS TeacherName "
                + "FROM Teachers t  "
                + "JOIN UserInformations inf "
                + "ON t.UserId = inf.UserId "
                + "WHERE TeacherId = ?";
